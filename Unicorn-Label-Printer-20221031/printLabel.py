...
dic_names = ["Black Plug Gateway, Ext Ant, CAN, Type A",
             ...
             "Black Plug Gateway, Ext Ant, Japan, Type A",
             "Black Plug Gateway, Ext Ant, Philippines, Type A",
             "Black Plug Gateway, Ext Ant, Thailand, Type A",
             "Black Plug Gateway, Ext Ant, Taiwan, Type A",
            ...
            "Green Wallplug, Japan, Type B",
            "Green Wallplug, Philippines, Type B",
            "Green Wallplug, Thailand, Type B",
            "Green Wallplug, Taiwan, Type B",
...
def printing():
   ...
   # messagebox.showinfo("Attention", "This Window is for printing out labels only.")
    ...
    def selnumber():
        ...
        if name.get() == "Black Plug Gateway, Ext Ant, Japan, Type A":
            number.set("GBP-2002-UA96-X")
        if name.get() == "Black Plug Gateway, Ext Ant, Philippines, Type A":
            number.set("GBP-2002-UA96-X")
        if name.get() == "Black Plug Gateway, Ext Ant, Thailand, Type A":
            number.set("GBP-2002-UA96-X")
        if name.get() == "Black Plug Gateway, Ext Ant, Taiwan, Type A":
            number.set("GBP-2002-UA96-X")
        ...
        if name.get() == "Green Wallplug, Japan, Type B":
            number.set("PGW-2003-UA96-I")
        if name.get() == "Green Wallplug, Philippines, Type B":
            number.set("PGW-2003-UA96-I")
        if name.get() == "Green Wallplug, Thailand, Type B":
            number.set("PGW-2003-UA96-I")
        if name.get() == "Green Wallplug, Taiwan, Type B":
            number.set("PGW-2003-UA96-I")
    ...
    def printlabel():
        ...
        elif qrcodeType[:2] == "0C":

            qr_value = qrcode.get()
            qr_text = qrcode.get()
            barcode_value = number.get()
            text_value = name.get()
            qr_path = pathlib.Path('./Premier tape label.label')

            printer_con = Dispatch('Dymo.DymoAddIn')
            printer_con.SelectPrinter(myPrinter)
            printer_con.Open(qr_path)
            printer_label = Dispatch('Dymo.DymoLabels')
            printer_label.SetField("BARCODE", qr_value)
            printer_label.SetField("TEXT_1", qr_value)
            # printer_label.SetField("BARCODE_2", imei)
            printer_con.setGraphicsAndBarcodePrintMode(OFF)
