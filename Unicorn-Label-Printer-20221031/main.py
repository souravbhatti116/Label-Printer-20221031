...
def main():
    ...
    def White_label():
        root.destroy()
        Whitetapeprint.boxlabel()

    printbtn = tk.Button(root, text="Infrastructure Label", command=lambda: print_label(), width=20, bg="aquamarine",
                        font=("Times new roman", 15), relief="ridge")
    printbtn.place(x=110, y=150)

    shipbtn = tk.Button(root, text="White Box Label", command=lambda: White_label(), width=20, bg="Light blue",
                        font=("Times new roman", 15), relief="ridge")
    shipbtn.place(x=110, y=200)

    exitbtn = tk.Button(root, text="Exit", command=lambda: sys.exit(), width=20, bg="orange red",
                        font=("Times new roman", 15), relief="ridge")
    exitbtn.place(x=110, y=250)
